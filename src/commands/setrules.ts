...
export const data = new SlashCommandBuilder()
  .setName("setrules")
  .setDescription(
    "Set the rules channel and the role to assign after acceptance.",
  )
  .addChannelOption((option) =>
    option.setName("channel").setDescription(
      "The channel to post the rules message",
    ).setRequired(true)
  )
  .addRoleOption((option) =>
    option.setName("role").setDescription(
      "The role to assign after rules are accepted",
    ).setRequired(true)
  );
...
export async function execute(interaction: CommandInteraction) {
  ...
  await interaction.reply(
    `Rules channel set to ${channel.toString()} and role set to ${role.toString()}.`,
  );
}
